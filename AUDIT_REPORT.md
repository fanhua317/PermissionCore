...
审阅目录：D:\学习\RBAC\PermissionCore

## 1. 结论摘要

本轮已完成代码、SQL、前端、Docker、脚本和文档的全量审阅与修复。基线中的 P0/P1 阻断项均已处理：认证改为 access/refresh 严格分型与单次轮换，所有授权变化通过持久化 `auth_version` 撤销旧 Token；ROLE_USER/ROLE_GUEST 默认不再获得后台管理查询权限；初始化事实源、数据库迁移、最小账户授权、Redis 边界、Swagger、头像代理、异常状态码和容器依赖顺序已经统一。
...
实现阶段实际执行了：现有库备份、幂等 migration/init、多次 Token 撤销验证、临时权限/用户回归、全新 Docker 数据卷初始化及审计项目容器/卷清理。没有删除现有业务数据卷，也没有重置现有管理员密码。
...
| 检查 | 结果 |
|---|---|
| `mvn clean verify` | 通过，30 项测试，0 失败/错误/跳过 |
...
| Docker 镜像 | backend/frontend 从源码构建通过；backend 构建内再次通过 30 项测试 |
| Docker 新空卷 | MySQL、Redis、backend healthy；frontend HTTP 200；db-access-init exited/0 |
| 现有 MySQL 8.0.46 | 备份后 migration/init 连续重跑通过；`auth_version`、6 个外键和 1 个 CHECK 正确 |
...
- `init-permissions.sql` 在同一事务内全局递增 `auth_version`，提交后旧 access/refresh token 立即被数据库门禁拒绝，不依赖缓存广播或重启。
- 直接执行其他 RBAC SQL 时，必须在同一事务中递增受影响用户的 `auth_version`；无法可靠确定范围时全局递增。
...
- schema.sql 已为新库包含 sys_user.auth_version、关系外键与禁止自继承检查；已有库使用幂等的 migrations/20260710_add_auth_version.sql 对齐同一契约，孤儿关系会阻断迁移，禁止靠删除数据卷碰运气。
- 独立 migrate-database.ps1 只执行该结构迁移，update-permissions.ps1 仍只执行 init 权限，避免两类变更相互掩盖。
...
- JWT：删除重复认证过滤器和 Token 日志；引入强密钥、access/refresh 类型、sessionId、单次 CAS 刷新与 DB `auth_version` 门禁。Redis 故障返回 503，不能伪装成凭据 401；前端遇到 503 不再清空会话。
- 授权一致性：用户状态、密码、角色分配、角色权限、继承、SoD 和权限初始化均持久递增受影响用户的 `auth_version`；旧 Redis 键无法绕过 DB 检查。
...
4. 已有库严格按备份 → 20260710 migration → init 权限 → 启动新版顺序。
5. migrate-database.ps1 只负责安全调用 migration；update-permissions.ps1 只负责安全调用 init，两者都不复制 SQL。
...
2. 停止写入流量。
3. 执行幂等的 20260710 数据库加固 migration（auth_version、关系外键、禁止自继承）。
...
- 固定基础镜像版本。
- MySQL healthcheck 同时验证 sys_user.auth_version 与启用的 ROLE_ADMIN/admin:* 基线，避免半初始化状态启动 backend；旧卷未迁移时仍保留 exec/cp 修复通道。Redis 使用带认证 PING；backend 的 `/api/health` 检查 DB 与 Redis 且不暴露详情；frontend 等待 backend healthy 后启动并检查本机静态首页。
...
- [x] 新空卷和已有卷升级各完成一次演练。
- [x] 已有库按备份 → migration → init 权限 → 新版启动顺序完成，auth_version 列验证通过且旧 JWT 失效。
