...
| 缓存 | Caffeine 本地缓存；Redis/Redisson 在 default/docker profile 可开启，dev profile 默认关闭 |
...
| 用户名 | 密码 | 说明 |
|--------|------|------|
| admin | Admin@123456 | 超级管理员（执行 `schema.sql` + `init-permissions.sql` 后可用） |
...
登录后，顶栏会显示当前激活角色。点击该入口可以勾选一个或多个已分配角色并保存：

- 系统只允许激活当前用户已分配且启用的角色；
- 保存时会校验 DSD 动态职责分离约束；
- 保存成功后会刷新 Token、菜单和路由权限；
- 如果切换后的角色不再拥有当前页面权限，系统会回到控制台。
